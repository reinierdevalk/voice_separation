...
#				epoch_x, epoch_y = x_train_new, y_train_new
				_, c = sess.run([optimizer, cost], feed_dict = {x: epoch_x, y: epoch_y, keep_prob: kp}) # JEO
				epoch_loss += c
...
#					acc_tr = accuracy.eval({x: x_train, y: y_train, keep_prob: kp}) ## HIERO
				...
				if val_perc != 0:
					...
					print('epoch: ', epoch, 'acc_val:', acc_val)
...
#				dothis = False
#				if dothis:
#					softmaxes_val = sess.run([softm, pred_class], feed_dict={x: x_val, keep_prob: 1.0})[0] # JEO
#					incorr = 0
